...
if [ ! -f $OHOS_SDK_NATIVE_DIR/llvm/bin/x86_64-unknown-linux-ohos-clang && ! -f $OHOS_SDK_NATIVE_DIR/llvm/bin/clang ]; then
...
onnxruntime_dir=onnxruntime-${SHERPA_ONNX_HARMONY_OS_TYPE}-x86_64-$onnxruntime_version
...
if [ -z $SHERPA_ONNX_LINK_OHOS_SYSTEM_LIBS ]; then
  SHERPA_ONNX_LINK_OHOS_SYSTEM_LIBS=ON
fi
...
cmake \
    ...
    -DSHERPA_ONNX_LINK_OHOS_SYSTEM_LIBS=$SHERPA_ONNX_LINK_OHOS_SYSTEM_LIBS \
