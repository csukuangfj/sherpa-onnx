...
def compute_mask(n_ctx, cur, num_cols):
    ans = torch.full((n_ctx, num_cols), float("-inf"))
    ans[:cur] = 0

    #  ans = torch.full((n_ctx, n_ctx), float("-inf"))
    #  ans[:cur, :cur] = torch.empty(cur, cur).fill_(float("-inf")).triu(1)
    return ans
...
def modified_qkv_attention(
    ...
    qk = q @ k
    if q.shape != v.shape and mask is not None:
...
#  MultiHeadAttention.qkv_attention = modified_qkv_attention
...
class ResidualAttentionBlockTensorCache(nn.Module):
    ...
    def forward(
        ...
        self_attn_x = self.attn(self.originalBlock.attn_ln(x), mask=mask)
        x = x + self_attn_x
...
class TextDecoderTensorCache(nn.Module):
    ...
    def forward(
        ...
        x = (
            ...
            + self.textDecoder.positional_embedding[0 : tokens.shape[-1]]
        )
...
def main():
    ...
    n_audio = mel.shape[0]
    tokens = torch.tensor([[0] * 12] * n_audio).to(mel.device)  # [n_audio, 3]
    ...
    logits = decoder(
        ...
        n_layer_cross_k,
        n_layer_cross_v,
        #  mask=compute_mask(decoder.n_ctx, 1, 1500),
    )
    ...
    tokens = torch.tensor([[1] * 12] * n_audio).to(mel.device)  # [n_audio, 1]
    ...
    torch.onnx.export(
        ...
        (
            ...
            n_layer_cross_k,
            n_layer_cross_v,
            #  compute_mask(decoder.n_ctx, 1, 1500),
        ),
        decoder_filename,
        opset_version=opset_version,
        input_names=[
